...
export default function SignInModal({ isOpen, onClose, reason = "copy this booking code and access live quantitative signals" }: SignInModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fadeIn">
      ...
      <div className="relative w-full max-w-md bg-[#0d0d10] border border-[var(--color-brand-emerald)]/40 rounded-2xl p-6 md:p-8 shadow-2xl shadow-emerald-950/50 z-10 flex flex-col gap-6">
        {/* Close Button */}
        <button
          onClick={onClose}
        ...
        <div className="flex flex-col items-center text-center gap-3">
          <div className="w-14 h-14 rounded-2xl bg-[var(--color-brand-emerald)]/15 border border-[var(--color-brand-emerald)]/30 flex items-center justify-center text-[var(--color-brand-emerald)] shadow-inner">
            <LockIcon size={28} />
          </div>
          ...
          <p className="text-zinc-300 text-sm font-mono leading-relaxed max-w-sm">
            Please sign in to <span className="text-[var(--color-brand-electricGreen)] font-bold">{reason}</span>.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <button
            ...
            disabled={loading}
            className="w-full py-3.5 px-4 rounded-xl font-bold text-sm bg-white text-black hover:bg-zinc-200 transition-all flex items-center justify-center gap-3 shadow-lg disabled:opacity-50"
          >
