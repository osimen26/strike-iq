...
function AreaChart({ data, color = "var(--primary-600)" }: { data: number[]; color?: string }) {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        <div className="w-12 h-12 border-4 border-white/10 border-t-[var(--primary-600)] rounded-full animate-spin mb-4"></div>
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 border-b border-zinc-900 pb-6">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary-600/20 border border-primary-600/40 mb-3 shadow-[0_0_15px_rgba(19,133,97,0.15)]">
            <span className="w-2 h-2 rounded-full bg-primary-600 animate-pulse"></span>
            <span className="text-xs font-mono font-bold text-primary-600 uppercase tracking-widest">VERIFIED MODEL METRICS</span>
          </div>
        ...
        {/* Trust badge */}
        <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-primary-600/10 border border-primary-600/30 shrink-0 shadow-sm">
      ...
      <div className="p-5 rounded-2xl bg-gradient-to-r from-[var(--primary-600)]/20 to-[var(--primary-600)]/5 border border-primary-600/40 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-[0_0_20px_rgba(19,133,97,0.1)]">
        ...
        <a 
          ...
          className="shrink-0 px-6 py-2.5 bg-[#10b981] hover:bg-[#059669] text-white text-xs font-mono font-bold rounded-full uppercase tracking-widest transition-colors shadow-sm"
        >
              ...
              <tbody className="divide-y divide-white/5">
                {proPicks.slice(0, 8).map((p, i) => (
                  <tr key={i} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="py-3 text-primary-100">{p.prediction}</td>
                    <td className="py-3 text-right">
                      <span className={`px-2 py-0.5 rounded text-xs font-bold ${p.confidence >= 80 ? "bg-primary-600/20 text-primary-100" : p.confidence >= 65 ? "bg-yellow-500/20 text-yellow-400" : "bg-red-500/20 text-red-400"}`}>
                        {p.confidence}%
                      </span>
                    </td>
                  </tr>
