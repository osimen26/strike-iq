        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-6 pb-4 border-b border-zinc-800/80">
          <div className="flex flex-wrap items-center gap-3">
            ...
            {match.isProPick && !match.isFreePick && (
              <span className="px-2.5 py-0.5 rounded bg-primary-600/15 border border-primary-600/30 text-[10px] font-mono font-bold text-primary-600 uppercase tracking-wider flex items-center gap-1">
          ...
          <div className="text-right font-mono text-xs text-zinc-400">
            <span className="text-primary-600 font-bold">{match.date}</span>
          ...
          {actuallyLocked && !isTeasing && (
            ...
            >
              <div className="w-9 h-9 rounded-xl bg-[#121215] border border-zinc-800 flex items-center justify-center mb-1 transition-transform">
                <LockIcon size={18} className="text-primary-600" />
              </div>
              <span className="text-xs font-mono font-bold text-white uppercase tracking-wider mb-0.5">
                {bettingPlatform} BOOKING CODE LOCKED
              </span>
              <span className="flex items-center gap-1 text-[11px] font-mono font-bold text-primary-600 tracking-widest">
                <ZapIcon size={12} />
                <span>
                  {requiresGuestAuth
              ...
              <div
                className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 border transition-colors ${
                  copied
                    ...
                    : "bg-primary-600/15 border-primary-600/30 text-primary-600"
                }`}
              >
                <TicketIcon size={24} />
              </div>
            ...
            <div
              className={`px-5 py-3 rounded-lg border font-mono font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-2.5 transition-all shrink-0 ${
                copied
                  ...
                  : "bg-primary-600 text-white border-emerald-400 group-hover/code:bg-emerald-600"
              }`}
            >
              <span>
          ...
          >
            <span
              className={`w-1.5 h-1.5 rounded-full ${
                actuallyLocked ? "bg-zinc-600" : "bg-primary-600 animate-pulse"
              }`}
            ></span>
