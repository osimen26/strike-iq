...
export default function AddPredictionPage() {
  ...
  const handleAddToDraftSlate = () => {
    ...
    const newDraft: DraftGame = {
      ...
      tags: tagsArray.length > 0 ? tagsArray : ["Pro Pick", "High EV"],
    };
    ...
    // Clear form for next game while preserving league/date/time defaults
    setFormData((prev) => ({
      ...prev,
      homeTeam: "",
      awayTeam: "",
  ...
  const handleAddCodeToQueue = () => {
    ...
    const newCodeItem: DraftCode = {
      ...
      title: formData.homeTeam.trim() || "VIP PRO SLIP",
      tags: tagsArray.length > 0 ? tagsArray : ["VIP Code", "Pro Slip"],
      notes: formData.analysis.trim() || `Exclusive Pro Member Booking Code on ${formData.bookmaker}.`,
    };

    setDraftCodes((prev) => [...prev, newCodeItem]);
    setCodePublishedMessage(`🎟️ Added to VIP Code Queue (${draftCodes.length + 1} codes staged)!`);
    setTimeout(() => setCodePublishedMessage(""), 5000);
    ...
    try {
      for (const codeItem of draftCodes) {
        const payload = {
          ...
          tags: ["VIP Code", "Booking Code", ...codeItem.tags],
        };
  ...
  // Publish Single Independent VIP Booking Code
  const handlePublishBookingCode = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/admin/predictions", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          homeTeam: formData.homeTeam.trim() || "VIP PRO SLIP",
          awayTeam: `${formData.bookmaker} CODE: ${formData.bookingCode.toUpperCase()}`,
          league: formData.league.trim() || "PRO FEED EXCLUSIVE",
          ...
          confidence: formData.confidence || 90,
          bookingCode: formData.bookingCode.toUpperCase(),
          bookmaker: formData.bookmaker,
          analysis: formData.analysis.trim() || `Exclusive Pro Member Booking Code on ${formData.bookmaker}.`,
          tags: ["VIP Code", "Booking Code", ...tagsArray],
        }),
      });
      ...
      setCodePublishedMessage("✅ VIP Booking Code Slip published independently to Pro Feed!");
      setSuccess(true);
  ...
  return (
    <div className="max-w-4xl mx-auto pb-16">
      ...
      {/* Mode Switcher Tabs */}
      <div className="flex bg-[#121215] border border-zinc-800 p-1.5 rounded-xl mb-8">
        <button
          type="button"
          onClick={() => {
            setPublishMode("game");
            ...
            {draftCodes.map((codeItem, idx) => (
              <div key={codeItem.id} className="flex items-center justify-between bg-[#0a0a0c] border border-zinc-800 rounded-lg p-4">
                <div className="flex items-center gap-4 min-w-0">
                  <span className="text-sm font-mono font-bold text-[var(--color-brand-emerald)]">#{idx + 1}</span>
                  <div className="min-w-0">
                    <div className="font-bold font-mono text-white tracking-wide">
                      {codeItem.bookingCode} <span className="text-[var(--color-brand-emerald)] font-sans text-xs">({codeItem.bookmaker})</span>
                    </div>
      ...
      {publishMode === "booking_code" ? (
        /* MODE 2: INDEPENDENT VIP BOOKING CODE SLIP WITH BATCH STAGING */
        <div className="bg-[#121215] border border-[var(--color-brand-emerald)]/40 rounded-xl p-6 space-y-6">
          <div className="flex items-center justify-between border-b border-zinc-800/80 pb-4">
            <div className="flex items-center gap-2">
              <span className="text-base font-bold text-white tracking-wide">🎟️ Publish Standalone Pro VIP Booking Code</span>
            </div>
